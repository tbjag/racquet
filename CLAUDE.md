...
- **State**: Phases 1–6 complete and deployed. Phase 7 frontend refactor complete (commits 1–9): theme infra, toast + `apiCall`, sidebar/chat/call/member-list extraction, WebSocket reconnect + `ConnectionBanner`, full visual styling pass, polish (transitions, scrollbar, animations, themed selection, reduced-motion). 29 Rust tests + 46 Playwright tests passing.
...
6. **Member list UI** — `frontend/src/lib/components/room/MemberList.svelte`. Backend's `room_users` broadcast excludes self, so the page builds a unified `members` derived array (self prepended with `isSelf: true`, plus `roomUsers` entries). Rendered under `RoomHeader` for now; the styling pass can move it. `data-testid` selectors `member-list` and `member-item`. 2 Playwright tests in `frontend/tests/members.spec.ts` covering self-only and join/leave updates (46 total now).
7. **WebSocket reconnect** — `frontend/src/lib/stores/connection.svelte.ts` exports a rune store with `status: 'closed'|'connecting'|'open'|'reconnecting'|'auth_failed'` plus `nextRetryAt`. `ws.ts` rewritten as a state machine: tracks `currentRoomId` itself (auto-set on `joinRoom`, cleared on `leaveRoom`), retries with exponential backoff (250 ms → 10 s, ×2 per attempt) and ±20 % jitter, gives up after 3 consecutive sockets that never reach OPEN and fires `onAuthFailure`. New `setToken(t)` method swaps the token and forces a reconnect; `+page.svelte`'s `handleSaveName` is now one line instead of a manual disconnect/reconnect/rejoin dance. `ConnectionBanner.svelte` (in `lib/components/chrome/`) shows during `connecting`/`reconnecting`/`auth_failed`. A `$effect` in `+page.svelte` tears down the active call when the socket leaves OPEN. No Playwright test — reconnect-loop timing under the test runner is too flaky to assert reliably; manually verified by killing/restarting the backend with `cargo run` and watching the banner cycle and rejoin.
8. **Visual styling pass** — populated scoped `<style>` blocks per component using only CSS variables from `app.css`. Sidebar: `bg-sunken` background, uppercase muted "Rooms" header with a square `+` button, `room-item` rows with `#` glyph + hover/active states, `UserProfile` row with ellipsis-truncated name and primary/ghost button variants, `CreateRoomForm` accent-coloured submit. Chat: `RoomHeader` gets a `#` glyph, `MessageList` highlights usernames in `--accent`, `MessageInput` rounded `--bg-elevated` field. Call: `CallControls` has `primary`/`danger`/`toggled` button variants; `CallStage` wraps content in a `.call-stage` container with a 50vh max-height screen-share area on top and a fixed-aspect tile grid below (240×135 normal, 160×90 in screen-share strip mode); each VideoTile is now wrapped in a `.tile` div with an absolutely-positioned overlay label ("You" / username). The remote-streams container uses `display: contents` so each peer's tile flows directly inside the cameras flexbox alongside the local tile. Auth: `login` is a centred 360px card with title + tagline + Google button, `auth/callback` is a centred muted message. Markup changes are additive (label spans, wrapper divs, `display: contents`) and preserve every existing `data-testid`; all 46 Playwright tests still green.
9. **Polish** — global niceties added to `app.css`: 120 ms ease transitions on `button`/`input`/`a` (background, border, color, opacity — *not* transform, so Playwright's actionability check stays happy), `button:active:not(:disabled)` 1 px translate, thin themed scrollbars (8 px, `--border` thumb hovering to `--border-strong`), themed `::selection` (`--accent` background), and a `prefers-reduced-motion` query that zeroes transition/animation durations. Toast appears with a 160 ms slide-up + fade-in (`@keyframes toast-in`), `ConnectionBanner` slides down from `-100%` over 200 ms (`@keyframes banner-in`) and gets a soft drop shadow + bumped to `z-index: 150` so it sits above the fixed `ThemeToggle`. Pure CSS — no markup, no testid changes.
